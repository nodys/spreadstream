 ...
 * @param  {String} config.id  spreadsheet id
 * @param  {String} config.sheet  spreadsheet sheet name
 * @param  {String} config.valueInputOption  See google sheet valueInputOption
 * @param  {Boolean} [config.replace] Replace current sheet value (default: false, append)
...
spreadstream.writableDocument = async function (config) {
  ...
  const valueInputOption = config.valueInputOption || enums.valueInputOption.USER_ENTERED
  const auth = config._auth || await spreadstream.google.authenticate(config.credential)
  ...
  async function push (rows) {
    ...
    rows = rows.map(row => {
      return row.map(value => Array.isArray(value) ? value.join('\n') : value)
    })
...
spreadstream.readDocument = async function (config) {
  ...
  const auth = config._auth || await spreadstream.google.authenticate(config.credential)
  const sheetsApi = spreadstream.google.sheets
  const valueRenderOption = config.valueRenderOption || enums.valueRenderOption.FORMATTED_VALUE
  ...
  const jsonRaw = config.jsonRaw
  const csvFieldToJson = config.csvFieldToJson || spreadstream.csvFieldToJson
  ...
  try {
    ...
    if (!jsonRaw) {
      values = values.map((row) => row.map((value, i) => csvFieldToJson(value, headers[i])))
    }
    return values
  } catch (ignore) {
    if (config.graceful) {
      ...
      throw new Error(`Unable to read input - check input and permission (${ignore.message}) - Use option "graceful" to ignore`)
    }
  }
}

spreadstream.csvFieldToJson = function (value, key) {
