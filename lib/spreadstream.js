...
function spreadstream (config) {
  ...
  const log = config.verbose
    ? m => console.log(chalk`[{green ${(new Date()).toISOString()}}] ${m}`)
    : m => m
  ...
  async function flush () {
    ...
    if (flush.count === 0) {
      ...
      streamType = Array.isArray(firstRow) ? spreadstream.type.ARRAY : spreadstream.type.OBJECT

      // Store headers
      if (!headers) {
        if (streamType === spreadstream.type.ARRAY) {
          headers = [...firstRow]
          buffer.shift()
        } else {
    ...
    for (let row of buffer) {
      if (streamType === spreadstream.type.OBJECT) {
...
// Statically Expose google api (for test mocking)
spreadstream.google = {
  /**
   * Sheet api
   * @type {Object}
   ...
   */
  authenticate: async function (credential) {
...
spreadstream.writableDocument = async function (config) {
  ...
  const valueInputOption = config.valueInputOption || spreadstream.inputOptions.USER_ENTERED
