...
async function initializer (argv) {
  const outputGlobal = join(basedir.config, 'varsync/config')
  const config = {
    id: argv.id,
    sheet: argv.sheet,
  ...
  console.log('  - ~/.config/spreadstream/config')
  console.log('  - See https://www.npmjs.com/package/rc#standards for other possible locations')
  console.log()
  console.log('----------------------------------------------------------------------------------------')
  console.log(chalk.bold(JSON.stringify(config, null, 2)))
  console.log('----------------------------------------------------------------------------------------')

  if (await confirm(`Should I write the config above, in the local rc file '${outputGlobal}'?`)) {
    await fs.outputJson(outputGlobal, config, { spaces: 2 })
  }
}
...
async function initCredentialOauth2 (argv) {
  ...
  console.log(chalk`{bold The 'Client ID' and the 'Client secret' below should have been sent to you by an system administrator}`)
  console.log(`(Or they may appear below as default value between parenthesis)`)
  console.log(chalk`{grey Else, if you now what you do, see the full instructions in the README https://github.com/nodys/spreadstream#create-oauth2-token}`)
  console.log()
